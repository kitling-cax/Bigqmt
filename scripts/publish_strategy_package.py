...
def _safe_relative(path: Path) -> str:
    ...
    if path.suffix.lower() in {".pyc", ".pyo"}:
        raise PackageError(f"generated Python bytecode is not publishable: {rel}")
...
def _validate_registry_entry(registry: dict[str, Any], strategy_id: str) -> dict[str, Any]:
    ...
    if entry.get("execution_enabled") is True:
        allowed_accounts = entry.get("allowed_accounts")
        if (entry.get("status") != "SIMULATION_ONLY" or
                not isinstance(allowed_accounts, list) or
                len(allowed_accounts) != 1 or
                not str(allowed_accounts[0]).strip()):
            raise PackageError("enabled package must name exactly one simulation-only account")
    return entry
...
def build_package(*, strategy_id: str, version: str, build_id: str, source: Path,
    ...
    for source_path in sorted(source.rglob("*")):
        ...
        if not source_path.is_file():
            continue
        # Generated bytecode is ignored rather than copied; it is not part of
        # a source release and would make hashes depend on the build machine.
        if "__pycache__" in source_path.relative_to(source).parts or source_path.suffix.lower() in {".pyc", ".pyo"}:
            continue
