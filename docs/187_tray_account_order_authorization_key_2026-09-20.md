...
2. **加入本账户授权 Key**：遮罩输入两次，最少 10 个字符，写入 Windows Credential Manager；
3. **设置/修改托盘删除密码**：遮罩输入两次，最少 8 个字符；本机只保存 SHA-256 散列；
4. **删除本账户授权 Key**：先输入托盘删除密码，再二次确认；删除后账户立即退回只读状态。

托盘主状态区持续显示 `下单授权 Key` 状态，每 30 秒刷新一次。状态变化写审计事件，但审计内容不含 Key。

建议使用密码管理器生成不少于 10 个字符的随机 Key，并在目标 Windows 机器上直接粘贴到托盘遮罩框。不要通过 Git、普通 NAS 文件或聊天传输。
