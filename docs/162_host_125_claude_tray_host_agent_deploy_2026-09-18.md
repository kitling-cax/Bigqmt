# `.125` 主机：VS Code（Claude）执行说明

你现在是在 `.125` Windows 策略运行主机执行任务。请严格按顺序完成，不要删除用户文件，不要修改 QMT 内的 `BIGQMT_BRIDGE` 策略，不要下单。
...
如果 `git status` 显示已有未提交改动，先报告文件列表，不得 reset、clean 或覆盖。确认工作区可操作后，使用 `.125` 结构发布分支 `host/125/structure`：

```powershell
git show-ref --verify --quiet refs/remotes/origin/host/125/structure
if ($LASTEXITCODE -eq 0) {
  git switch host/125/structure 2>$null
  if ($LASTEXITCODE -ne 0) { git switch --track origin/host/125/structure }
} else {
  git switch -c host/125/structure --track origin/feature/nas-private-config
}
git pull --ff-only
```
...
```json
{
  ...
  "coordinator": {
    ...
    "host_id": "<HOST_125_ID_FROM_LOCAL_CONFIG>"
  },
  "host_agent": {
...
确认以下文件存在；缺少 Fact Secret 时停止并报告，不得从 `.113` 复制：
...
git push -u origin host/125/structure
```
