...
如果 `git status` 显示已有未提交改动，先报告文件列表，不得 reset、clean 或覆盖。确认工作区可操作后，使用 `.125` 主机分支：

```powershell
git show-ref --verify --quiet refs/remotes/origin/host/125
if ($LASTEXITCODE -eq 0) {
  git switch host/125 2>$null
  if ($LASTEXITCODE -ne 0) { git switch --track origin/host/125 }
} else {
  git switch -c host/125 --track origin/main
}
git pull --ff-only
```
