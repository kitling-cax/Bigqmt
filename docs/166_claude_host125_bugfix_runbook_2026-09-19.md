# `.125` Claude 执行单：Host 修复与托盘重编译

> 目标：在 `.125` 本机完成代码同步、编译、测试和现场只读验证。
...
期望看到修复提交：`12442ca fix: repair host fact identity redis paths and deterministic trays`。
...
`.125` 的 `host_id` 必须是本机实际值（通常为 `192.168.1.125`）；不要使用 `192.0.2.125` 这类测试占位值。

不要从 `.105` 复制账户密码、Fact Secret 或授权文件。它们必须是 `.125` 独立生成的本机文件。
...
正式 profile 同样检查，但不要因此放开正式下单。若输出 `FACT_HOST_ID_MISMATCH`，停止托盘并重新签发 `.125` 专属 Secret；不要改 `machine.local.json` 去迁就错误 Secret。
