...
log = logging.getLogger("kitling_bigqmt.qmt_launcher")
...
def login_existing_qmt(credentials=None, window_title_prefix=None):
    """Complete a visible full-QMT login dialog without spawning a second QMT.

    A previous automatic attempt may already have started ``XtItClient.exe``
    but stopped safely at the dialog.  Restarting it only loses time and can
    create port churn; reuse that specific, visible dialog instead.
    """
    _login_via_window(credentials or {}, window_title_prefix)


def qmt_window_state(window_title_prefix=None):
    """Return ``LOGIN_DIALOG``, ``MAIN_WINDOW`` or ``NO_WINDOW``.

    This is profile-local window evidence.  It deliberately does not infer
    account readiness from FormulaServer because that port is shared when two
    QMT installations run on the same workstation.
    """
    try:
        import win32gui
        import ctypes
    ...
    def _collect(hwnd, acc):
        if not win32gui.IsWindowVisible(hwnd):
            return
        title = win32gui.GetWindowText(hwnd) or ""
        if win32gui.GetClassName(hwnd) == "Qt5QWindowIcon" and prefix in title:
            acc.append(hwnd)
    ...
    for hwnd in matches:
        ...
        if _looks_like_login_window(rect, user32.GetSystemMetrics(0), user32.GetSystemMetrics(1)):
            return "LOGIN_DIALOG"
    return "MAIN_WINDOW"
...
def _login_via_window(credentials, window_title_prefix=None, appear_timeout_seconds=90.0):
    ...
    This path is strictly for the full BigQMT terminal: it never clicks the
    ``独立交易`` checkbox.  On 国金 QMT that checkbox selects MiniQMT; leaving
    it unchecked is therefore a safety invariant, not a UI convenience.
    ...
    def _region_pixels(region):
        """Read a small physical-screen region for focus verification.

        ``pyautogui.screenshot(region=...)`` delegates to Pillow with
        ``all_screens=False``.  On this workstation that Win32 capture path
        raises ``screen grab failed`` whenever an auxiliary display is
        present, although the all-screens capture path works.  A failed visual
        proof must still fail closed; this fallback simply uses the working
        physical-desktop capture mode and maps the requested desktop rectangle
        to its virtual-screen image coordinates.
        """
        ...
        crop = (left - virtual_left, top - virtual_top,
                left - virtual_left + width, top - virtual_top + height)
        if crop[0] < 0 or crop[1] < 0 or crop[2] > image.width or crop[3] > image.height:
            raise QmtLauncherError("password focus verification region is outside the captured desktop")
        return list(image.crop(crop).convert("L").getdata())
