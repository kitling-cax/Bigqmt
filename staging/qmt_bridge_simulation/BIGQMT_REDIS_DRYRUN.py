...
try:
    ...
    # Bind an order-capable release to the physical QMT Python directory from
    # which this mounted entry actually loaded.  A simulation local config
    # copied into the formal terminal therefore fails closed instead of merely
    # trusting its own environment label.
    BIGQMT_REDIS_CONFIG = dict(BIGQMT_REDIS_CONFIG or {})
    _execution_admission = dict(
        BIGQMT_REDIS_CONFIG.get("execution_admission") or {})
    _execution_admission["detected_qmt_python_dir"] = _SOURCE_ROOT
    BIGQMT_REDIS_CONFIG["execution_admission"] = _execution_admission
...
try:
    ...
    if not BIGQMT_ACCOUNT_ID:
        # A read-only production profile may intentionally leave the account
        # blank and use the account selected in QMT's model-trading UI.  The
        # mounted entry owns that injected global; preserve it here instead of
        # configuring an empty RPC service.
        BIGQMT_ACCOUNT_ID = _fallback_account_id()
