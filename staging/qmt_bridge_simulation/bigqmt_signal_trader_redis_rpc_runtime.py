...
# None means keep the strategy package's normal read-only method set.  A
# profile may supply a smaller tuple for a formal read-only deployment.
RPC_ALLOWED_METHODS = None
...
RPC_ALLOWED_METHODS = BIGQMT_REDIS_CONFIG.get("rpc_allowed_methods", RPC_ALLOWED_METHODS)
if RPC_ALLOWED_METHODS is not None:
    RPC_ALLOWED_METHODS = tuple(RPC_ALLOWED_METHODS)
EXECUTION_ADMISSION = dict(BIGQMT_REDIS_CONFIG.get("execution_admission") or {})
...
def _apply_config(account_id):
    ...
    if RPC_ALLOWED_METHODS is not None:
        rpc_block["allowed_methods"] = RPC_ALLOWED_METHODS
    ...
    configure(
        ...
        # Top-level because AdapterFactory builds the guarded broker gateway
        # from the application config.  Keeping it only under rpc would make
        # every future release remain locked even after valid approval.
        execution_admission=EXECUTION_ADMISSION,
...
def configure_runtime_redis(redis_config):
    ...
    RPC_ALLOWED_METHODS = redis_config.get("rpc_allowed_methods", RPC_ALLOWED_METHODS)
    if RPC_ALLOWED_METHODS is not None:
        RPC_ALLOWED_METHODS = tuple(RPC_ALLOWED_METHODS)
    EXECUTION_ADMISSION = dict(redis_config.get("execution_admission") or {})
